...
[mu_a(1,1)]=Rheology_TP(mu_a_0(1,1),T_0,P_0,T_a(1,1),P_a(1,1)); % 环空出口流体粘度，Pa*s
[Ff_a(1,1),flow_pattern_a(1,1)]=Friction_annulus(rho_a(1,1),V_a(1,1),mu_a(1,1),D_w(1),D_d_o(1)); % 环空出口单位长度摩擦压降（Pa/m）、流体流态（层流1、过渡流2、湍流3）
...
for t=2:1:(2*Nt-1)
    ...
    while abs(ERROR_AnnOutPressure)>epsilon_e && COUNT_AnnOutPressure<epsilon_t
        ...
        [mu_a(t,Nx)]=Rheology_TP(mu_a_0(t,Nx),T_0,P_0,T_a(t,Nx),P_a(t,Nx)); % 环空井底流体粘度（Pa*s）
        [Ff_a(t,Nx),flow_pattern_a(t,Nx)]=Friction_annulus(rho_a(t,Nx),V_a(t,Nx),mu_a(t,Nx),D_w(Nx),D_d_o(Nx)); % 环空井底单位长度摩擦压降（Pa/m）、流体流态（层流1、过渡流2、湍流3）
...
[mu_d(1,Nx)]=Rheology_TP(mu_d_0(1,Nx),T_0,P_0,T_d(1,Nx),P_d(1,Nx)); % 管内井底流体粘度，Pa*s
[Ff_d(1,Nx),flow_pattern_d(1,Nx)]=Friction_drillpipe(rho_d(1,Nx),V_d(1,Nx),mu_d(1,Nx),D_d_i(Nx)); % 管内井底单位长度摩擦压降（Pa/m）、流体流态（层流1、过渡流2、湍流3）
...
for t=2:1:(2*Nt-1)
    ...
    while abs(ERROR_PipOutPressure)>epsilon_e && COUNT_PipOutPressure<epsilon_t
        ...
        [mu_d(t,1)]=Rheology_TP(mu_d_0(t,1),T_0,P_0,T_d(t,1),P_d(t,1)); % 入口流体粘度，Pa*s
        [Ff_d(t,1),flow_pattern_d(t,1)]=Friction_drillpipe(rho_d(t,1),V_d(t,1),mu_d(t,1),D_d_i(1)); % 入口单位长度摩擦压降（Pa/m）、流体流态（层流1、过渡流2、湍流3）
