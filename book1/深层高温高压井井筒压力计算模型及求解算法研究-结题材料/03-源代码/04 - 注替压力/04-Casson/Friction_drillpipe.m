function [Ff_d,flow_pattern_d]=Friction_drillpipe(rho_d,V_d,tau_y_c_d,eta_infinity_d,D_d_i,Qv_d)
    % 根据钻杆结构、钻井液性质和流动参数，计算钻杆摩擦压降及流型
    % 输入参数：rho_d为钻杆中钻井液密度（kg/m^3），V_d为钻杆中钻井液流速（m/s），eta_infinity_d为钻杆中钻井液的卡森黏度（Pa·s），tau_y_c_d为钻杆中钻井液的卡森屈服应力（Pa），D_d_i为钻杆内径（m），Qv_d为体积流量（m^3/s）
    ...
    while abs(err_tau)>1e-3 && loop<=1000 % 迭代终止条件
        int=(4*Qv_d*eta_infinity_d)/(pi*(D_d_i/2)^3)-(4*tau_y_c_d)/3+(16*(tau_y_c_d*tau_w_old)^0.5)/7+(tau_y_c_d^4)/(21*tau_w_old^3);
        tau_w_new=int;
    ...
    if Re_g_d<=Re_gcl_d % 层流
        ...
        Ff_d=2*f_d*rho_d*V_d^2/D_d_i; % 计算层流条件下，钻杆内钻井液在单位长度上的摩擦压降
        flow_pattern_d=1; % flow_pattern_d为钻杆流型，1表示层流
    elseif Re_g_d>Re_gct_d % 湍流
        f_d_old=100; % 迭代初值
        err_f=1;
        loop=1;
        while abs(err_f)>1e-3 && loop<=1000 % 迭代终止条件
