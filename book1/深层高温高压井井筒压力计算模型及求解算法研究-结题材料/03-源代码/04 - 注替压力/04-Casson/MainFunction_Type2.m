...
[tau_y_c_d(1,1),eta_infinity_d(1,1)]=Rheology_TP(tau_y_c_d_0(1,1),eta_infinity_d_0(1,1),T_0,P_0,T_d(1,1),P_d(1,1)); % 管内入口流体卡森屈服应力（Pa）、卡森粘度（Pa*s）
[Ff_d(1,1),flow_pattern_d(1,1)]=Friction_drillpipe(rho_d(1,1),V_d(1,1),tau_y_c_d(1,1),eta_infinity_d(1,1),D_d_i(1),Qv_d(1,1)); % 管内入口单位长度摩擦压降（Pa/m）、流体流态（层流1、过渡流2、湍流3）
...
for t=2:1:(2*Nt-1)
    ...
    while abs(ERROR_PipInPressure)>epsilon_e && COUNT_PipInPressure<epsilon_t
        ...
        [tau_y_c_d(t,Nx),eta_infinity_d(t,Nx)]=Rheology_TP(tau_y_c_d_0(t,Nx),eta_infinity_d_0(t,Nx),T_0,P_0,T_d(t,Nx),P_d(t,Nx)); % 管内井底流体卡森屈服应力（Pa）、卡森粘度（Pa*s）
        [Ff_d(t,Nx),flow_pattern_d(t,Nx)]=Friction_drillpipe(rho_d(t,Nx),V_d(t,Nx),tau_y_c_d(t,Nx),eta_infinity_d(t,Nx),D_d_i(Nx),Qv_d(t,Nx)); % 管内井底单位长度摩擦压降（Pa/m）、流体流态（层流1、过渡流2、湍流3）
...
[tau_y_c_a(1,Nx),eta_infinity_a(1,Nx)]=Rheology_TP(tau_y_c_a_0(1,Nx),eta_infinity_a_0(1,Nx),T_0,P_0,T_a(1,Nx),P_a(1,Nx)); % 环空井底流体卡森屈服应力（Pa）、卡森粘度（Pa*s）
[Ff_a(1,Nx),flow_pattern_a(1,Nx)]=Friction_annulus(rho_a(1,Nx),V_a(1,Nx),tau_y_c_a(1,Nx),eta_infinity_a(1,Nx),D_w(Nx),D_d_o(Nx),Qv_a(1,Nx)); % 环空井底单位长度摩擦压降（Pa/m）、流体流态（层流1、过渡流2、湍流3）
...
for t=2:1:(2*Nt-1)
    ...
    while abs(ERROR_AnnInPressure)>epsilon_e && COUNT_AnnInPressure<epsilon_t
        ...
        [tau_y_c_a(t,1),eta_infinity_a(t,1)]=Rheology_TP(tau_y_c_a_0(t,1),eta_infinity_a_0(t,1),T_0,P_0,T_a(t,1),P_a(t,1)); % 环空出口流体卡森屈服应力（Pa）、卡森粘度（Pa*s）
        [Ff_a(t,1),flow_pattern_a(t,1)]=Friction_annulus(rho_a(t,1),V_a(t,1),tau_y_c_a(t,1),eta_infinity_a(t,1),D_w(1),D_d_o(1),Qv_a(t,1)); % 环空出口单位长度摩擦压降（Pa/m）、流体流态（层流1、过渡流2、湍流3）
