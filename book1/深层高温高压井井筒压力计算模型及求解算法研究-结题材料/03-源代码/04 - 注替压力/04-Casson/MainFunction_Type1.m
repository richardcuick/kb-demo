...
[tau_y_c_0_1,eta_infinity_0_1]=Rheology_Casson(data_rheology_01); % 计算第一种流体T_0、P_0下的卡森屈服应力（Pa）、卡森粘度（Pa*s）
[tau_y_c_0_2,eta_infinity_0_2]=Rheology_Casson(data_rheology_02); % 计算第二种流体T_0、P_0下的卡森屈服应力（Pa）、卡森粘度（Pa*s）
...
for t=1:1:(2*Nt-1)
    for x=1:1:Nx
        ...
        tau_y_c_a_0(t,x)=tau_y_c_0_1; % 初始时刻环空第t个时间节点第x个空间节点处流体T_0、P_0下的卡森屈服应力，Pa
        tau_y_c_d_0(t,x)=tau_y_c_0_1; % 初始时刻管内第t个时间节点第x个空间节点处流体T_0、P_0下的卡森屈服应力，Pa
        eta_infinity_a_0(t,x)=eta_infinity_0_1; % 初始时刻环空第t个时间节点第x个空间节点处流体T_0、P_0下的卡森粘度，Pa*s
        eta_infinity_d_0(t,x)=eta_infinity_0_1; % 初始时刻管内第t个时间节点第x个空间节点处流体T_0、P_0下的卡森粘度，Pa*s
    end
end
...
for t=1:1:Nt
    for x=1:1:Nx
        if x<=t
            ...
            tau_y_c_d_0(t,x)=tau_y_c_0_2; % 管内第t个时间节点第x个空间节点处流体T_0、P_0下的卡森屈服应力，Pa
            eta_infinity_d_0(t,x)=eta_infinity_0_2; % 管内第t个时间节点第x个空间节点处流体T_0、P_0下的卡森粘度，Pa*s
        else
            rho_d_0(t,x)=rho_0_1; % 管内第t个时间节点第x个空间节点处流体T_0、P_0下的密度，kg/m^3
            tau_y_c_d_0(t,x)=tau_y_c_0_1; % 管内第t个时间节点第x个空间节点处流体T_0、P_0下的卡森屈服应力，Pa
            eta_infinity_d_0(t,x)=eta_infinity_0_1; % 管内第t个时间节点第x个空间节点处流体T_0、P_0下的卡森粘度，Pa*s
        end
    end
end
...
for t=Nt:1:(2*Nt-1)
    for x=Nx:-1:1
        if x>=(2*Nt-t) %% Nx-x<=t-Nt
            ...
            tau_y_c_a_0(t,x)=tau_y_c_0_2; % 环空第t个时间节点第x个空间节点处流体T_0、P_0下的卡森屈服应力，Pa
            eta_infinity_d_0(t,x)=eta_infinity_0_2; % 管内第t个时间节点第x个空间节点处流体T_0、P_0下的卡森粘度，Pa*s
        else
            rho_a_0(t,x)=rho_0_1; % 环空第t个时间节点第x个空间节点处流体T_0、P_0下的密度，kg/m^3
            tau_y_c_a_0(t,x)=tau_y_c_0_1; % 环空第t个时间节点第x个空间节点处流体T_0、P_0下的卡森屈服应力，Pa
            eta_infinity_d_0(t,x)=eta_infinity_0_1; % 管内第t个时间节点第x个空间节点处流体T_0、P_0下的卡森粘度，Pa*s
        end
        rho_d_0(t,x)=rho_0_2; % 管内第t个时间节点第x个空间节点处流体T_0、P_0下的密度，kg/m^3
        tau_y_c_d_0(t,x)=tau_y_c_0_2; % 管内第t个时间节点第x个空间节点处流体T_0、P_0下的卡森屈服应力，Pa
...
OutPressure=0.8*10^6; % 环空出口压力，Pa
...
[tau_y_c_a(1,1),eta_infinity_a(1,1)]=Rheology_TP(tau_y_c_a_0(1,1),eta_infinity_a_0(1,1),T_0,P_0,T_a(1,1),P_a(1,1)); % 环空出口流体卡森屈服应力（Pa）、卡森粘度（Pa*s）
[Ff_a(1,1),flow_pattern_a(1,1)]=Friction_annulus(rho_a(1,1),V_a(1,1),tau_y_c_a(1,1),eta_infinity_a(1,1),D_w(1),D_d_o(1),Qv_a(1,1)); % 环空出口单位长度摩擦压降（Pa/m）、流体流态（层流1、过渡流2、湍流3）
...
for x=2:1:Nx
    ...
    while abs(ERROR_AnnPressure)>epsilon_e && COUNT_AnnPressure<epsilon_t
        ...
        [tau_y_c_a(1,x),eta_infinity_a(1,x)]=Rheology_TP(tau_y_c_a_0(1,x),eta_infinity_a_0(1,x),T_0,P_0,T_a(1,x),P_a_ass(1,x)); % 环空流体卡森屈服应力（Pa）、卡森粘度（Pa*s）
        [Ff_a(1,x),flow_pattern_a(1,x)]=Friction_annulus(rho_a(1,x),V_a(1,x),tau_y_c_a(1,x),eta_infinity_a(1,x),D_w(x),D_d_o(x),Qv_a(1,x)); % 环空单位长度摩擦压降（Pa/m）、流体流态（层流1、过渡流2、湍流3）
...
for t=2:1:(2*Nt-1)
    ...
    while abs(ERROR_AnnOutPressure)>epsilon_e && COUNT_AnnOutPressure<epsilon_t
        ...
        [tau_y_c_a(t,Nx),eta_infinity_a(t,Nx)]=Rheology_TP(tau_y_c_a_0(t,Nx),eta_infinity_a_0(t,Nx),T_0,P_0,T_a(t,Nx),P_a(t,Nx)); % 环空井底流体卡森屈服应力（Pa）、卡森粘度（Pa*s）
        [Ff_a(t,Nx),flow_pattern_a(t,Nx)]=Friction_annulus(rho_a(t,Nx),V_a(t,Nx),tau_y_c_a(t,Nx),eta_infinity_a(t,Nx),D_w(Nx),D_d_o(Nx),Qv_a(t,Nx)); % 环空井底单位长度摩擦压降（Pa/m）、流体流态（层流1、过渡流2、湍流3）
        ...
        for x=Nx-1:-1:1
            ...
            while abs(ERROR_AnnPressure)>epsilon_e && COUNT_AnnPressure<epsilon_t
                ...
                [tau_y_c_a(t,x),eta_infinity_a(t,x)]=Rheology_TP(tau_y_c_a_0(t,x),eta_infinity_a_0(t,x),T_0,P_0,T_a(t,x),P_a_ass(t,x)); % 环空流体卡森屈服应力（Pa）、卡森粘度（Pa*s）
                [Ff_a(t,x),flow_pattern_a(t,x)]=Friction_annulus(rho_a(t,x),V_a(t,x),tau_y_c_a(t,x),eta_infinity_a(t,x),D_w(x),D_d_o(x),Qv_a(t,x)); % 环空流体单位长度摩擦压降（Pa/m）、流体流态（层流1、过渡流2、湍流3）
...
[tau_y_c_d(1,Nx),eta_infinity_d(1,Nx)]=Rheology_TP(tau_y_c_d_0(1,Nx),eta_infinity_d_0(1,Nx),T_0,P_0,T_d(1,Nx),P_d(1,Nx)); % 管内井底流体卡森屈服应力（Pa）、卡森粘度（Pa*s）
[Ff_d(1,Nx),flow_pattern_d(1,Nx)]=Friction_drillpipe(rho_d(1,Nx),V_d(1,Nx),tau_y_c_d(1,Nx),eta_infinity_d(1,Nx),D_d_i(Nx),Qv_d(1,Nx)); % 管内井底单位长度摩擦压降（Pa/m）、流体流态（层流1、过渡流2、湍流3）
...
for x=Nx-1:-1:1
    ...
    while abs(ERROR_PipPressure)>epsilon_e && COUNT_PipPressure<epsilon_t
        ...
        [tau_y_c_d(1,x),eta_infinity_d(1,x)]=Rheology_TP(tau_y_c_d_0(1,x),eta_infinity_d_0(1,x),T_0,P_0,T_d(1,x),P_d_ass(1,x)); % 管内流体卡森屈服应力（Pa）、卡森粘度（Pa*s）
        [Ff_d(1,x),flow_pattern_d(1,x)]=Friction_drillpipe(rho_d(1,x),V_d(1,x),tau_y_c_d(1,x),eta_infinity_d(1,x),D_d_i(x),Qv_d(1,x)); % 管内单位长度摩擦压降（Pa/m）、流体流态（层流1、过渡流2、湍流3）
...
for t=2:1:(2*Nt-1)
    ...
    while abs(ERROR_PipOutPressure)>epsilon_e && COUNT_PipOutPressure<epsilon_t
        ...
        [tau_y_c_d(t,1),eta_infinity_d(t,1)]=Rheology_TP(tau_y_c_d_0(t,1),eta_infinity_d_0(t,1),T_0,P_0,T_d(t,1),P_d(t,1)); % 入口流体卡森屈服应力（Pa）、卡森粘度（Pa*s）
        [Ff_d(t,1),flow_pattern_d(t,1)]=Friction_drillpipe(rho_d(t,1),V_d(t,1),tau_y_c_d(t,1),eta_infinity_d(t,1),D_d_i(1),Qv_d(t,1)); % 入口单位长度摩擦压降（Pa/m）、流体流态（层流1、过渡流2、湍流3）
        ...
        for x=2:1:Nx
            ...
            while abs(ERROR_PipPressure)>epsilon_e && COUNT_PipPressure<epsilon_t
                ...
                [tau_y_c_d(t,x),eta_infinity_d(t,x)]=Rheology_TP(tau_y_c_d_0(t,x),eta_infinity_d_0(t,x),T_0,P_0,T_d(t,x),P_d_ass(t,x)); % 管内流体卡森屈服应力（Pa）、卡森粘度（Pa*s）
                [Ff_d(t,x),flow_pattern_d(t,x)]=Friction_drillpipe(rho_d(t,x),V_d(t,x),tau_y_c_d(t,x),eta_infinity_d(t,x),D_d_i(x),Qv_d(t,x)); % 管内单位长度摩擦压降（Pa/m）、流体流态（层流1、过渡流2、湍流3）
