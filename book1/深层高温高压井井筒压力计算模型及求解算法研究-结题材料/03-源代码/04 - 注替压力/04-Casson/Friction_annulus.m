function [Ff_a,flow_pattern_a]=Friction_annulus(rho_a,V_a,tau_y_c_a,eta_infinity_a,D_w,D_d_o,Qv_a)
    % 根据环空结构、钻井液性质和流动参数，计算环空摩擦压降及流型
    % 输入参数：rho_a为环空中钻井液密度（kg/m^3），V_a为环空中钻井液流速（m/s），eta_infinity_a为环空中钻井液的卡森黏度（Pa·s），tau_y_c_a为环空中钻井液的卡森屈服应力（Pa），D_w为井筒内径（m），D_d_o为钻杆外径（m），Qv_a为体积流量（m^3/s）
    ...
    loop=1;
    err_tau=1;
    while abs(err_tau)>1e-3 && loop<=1000 % 迭代终止条件
        int=(6*Qv_a*eta_infinity_a)/(pi*(D_w/2+D_d_o/2)*(D_h/2)^2)-(3*tau_y_c_a)/2+(5*tau_y_c_a^3)/(2*tau_w_old^2)+(12*tau_y_c_a^0.5*(tau_w_old^2.5-tau_y_c_a^2.5))/(5*tau_w_old^2);
        tau_w_new=int;
    ...
    mu_aw=tau_w/gammadot_w; % 壁面表观粘度，Pa*s
    ...
    if Re_g_a<=Re_gcl_a % 层流
        ...
        Ff_a=2*f_a*rho_a*V_a^2/D_h; % 计算层流条件下，环空内钻井液在单位长度上的摩擦压降
    ...
    elseif Re_g_a>Re_gct_a % 湍流
        ...
        Ff_a=2*f_a*rho_a*V_a^2/D_h; % 计算湍流条件下，环空内钻井液在单位长度上的摩擦压降
        ...
        Ff_a=2*f_a*rho_a*V_a^2/D_h; % 计算过渡流条件下，环空内钻井液在单位长度上的摩擦压降
