...
% 输入参数：welldepth为井深（m），rho_0为钻井液密度（kg/m^3），mu_0为钻井液粘度（Pa·s），Qv_0为钻井液排量（m^3/s）
% 输出参数：T_Annulus为环空钻井液温度（℃），T_Drillpipe为管内钻井液温度（℃），Depth为空间节点（m），Time为时间节点（s）

delta_t=3600;           % 时间步长，s
timesteps=120;          % 循环次数，即总模拟时间为delta_t×timesteps

dz=40;                  % 以dz为主要的网格单元长度，m
...
Rheology_type=1;        % 1为牛顿流体，2为幂律流体
% Rheology_type=2;        % 1为牛顿流体，2为幂律流体

%% 井身结构处理及网格划分
nobox_r=200;    % 径向网格单元数

for i=1:casing_number   % 套管序号，序号从内向外增加，即casing_number=1为生产套管
    Depth_casing(i)=welldepth-4000/4*i;     % 套管下深，m
    if i==casing_number                     % 最外层套管（表层套管）
        Depth_fluid(i)=0;                   % 表层套管水泥返深，m（一般返至地面）
    else
        Depth_fluid(i)=Depth_casing(i)-150; % 非表层套管水泥返深，m（一般返至密封点以上50~200米）
    end
end
...
%% 网格参数赋值
[rou_m,Cpm,lambdam]=deal(rho_0,1600,1.75);       % 钻井液的密度（kg/m^3）、比热容（J/(kg·℃））、导热系数（W/(m·℃））
