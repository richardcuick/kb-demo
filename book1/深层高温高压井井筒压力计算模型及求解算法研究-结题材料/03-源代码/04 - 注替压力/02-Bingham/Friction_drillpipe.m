function [Ff_d,flow_pattern_d]=Friction_drillpipe(rho_d,V_d,tau_y,mu_p_d,D_d_i,Qv_d)
    % 根据钻杆结构、钻井液性质和流动参数，计算钻杆摩擦压降及流型
    % 输入参数：rho_d为钻杆中钻井液密度（kg/m^3），V_d为钻杆中钻井液流速（m/s），mu_p_d为钻杆中钻井液的塑性黏度（Pa·s），tau_y为钻杆中钻井液的屈服应力（Pa），D_d_i为钻杆内径（m），Qv_d为钻井液体积流量（m^3/s）
    ...
    while abs(err_tau)>1e-4 % 壁面处屈服应力收敛的判断标准
        tau_w_new=4*mu_p_d*Qv_d/(pi*(D_d_i/2)^3)+4/3*tau_y-1/3*tau_y^4/tau_w_old^3; % 计算新的壁面屈服应力值
