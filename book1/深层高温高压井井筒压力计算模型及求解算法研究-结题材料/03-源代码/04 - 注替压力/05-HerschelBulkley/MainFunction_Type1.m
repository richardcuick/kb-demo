%% 流体流动方向计算（给定入口流量、出口压力）
close all;clear all;clc;
%% 基础数据及预处理（国际单位制）
T_0=20; % 流体性质（密度、流变参数）测试温度，℃
P_0=0.1*10^5; % 流体性质（密度、流变参数）测试压力，Pa
data_rheology_01=csvread('RheologyData_01.csv',1,0); % 导入第一种流体T_0、P_0下的流变性测试数据
data_rheology_02=csvread('RheologyData_02.csv',1,0); % 导入第二种流体T_0、P_0下的流变性测试数据
[tau_y_0_1,K_0_1,n_0_1]=Rheology_HerschelBulkley(data_rheology_01); % 计算第一种流体T_0、P_0下的屈服应力（Pa）、稠度系数（Pa*s^n）、流性指数
[tau_y_0_2,K_0_2,n_0_2]=Rheology_HerschelBulkley(data_rheology_02); % 计算第二种流体T_0、P_0下的屈服应力（Pa）、稠度系数（Pa*s^n）、流性指数
...
for t=1:1:(2*Nt-1)
    for x=1:1:Nx
        ...
        tau_y_a_0(t,x)=tau_y_0_1; % 初始时刻环空第t个时间节点第x个空间节点处流体T_0、P_0下的屈服应力，Pa
        tau_y_d_0(t,x)=tau_y_0_1; % 初始时刻管内第t个时间节点第x个空间节点处流体T_0、P_0下的屈服应力，Pa
        K_a_0(t,x)=K_0_1; % 初始时刻环空第t个时间节点第x个空间节点处流体T_0、P_0下的稠度系数，Pa*s^n
        K_d_0(t,x)=K_0_1; % 初始时刻管内第t个时间节点第x个空间节点处流体T_0、P_0下的稠度系数，Pa*s^n
        n_a_0(t,x)=n_0_1; % 初始时刻环空第t个时间节点第x个空间节点处流体T_0、P_0下的流性指数
        n_d_0(t,x)=n_0_1; % 初始时刻管内第t个时间节点第x个空间节点处流体T_0、P_0下的流性指数
    end
end
...
for t=1:1:Nt
    for x=1:1:Nx
        if x<=t
            ...
            tau_y_d_0(t,x)=tau_y_0_2; % 管内第t个时间节点第x个空间节点处流体T_0、P_0下的屈服应力，Pa
            K_d_0(t,x)=K_0_2; % 管内第t个时间节点第x个空间节点处流体T_0、P_0下的稠度系数，Pa*s^n
            n_d_0(t,x)=n_0_2; % 管内第t个时间节点第x个空间节点处流体T_0、P_0下的流性指数
        else
            rho_d_0(t,x)=rho_0_1; % 管内第t个时间节点第x个空间节点处流体T_0、P_0下的密度，kg/m^3
            tau_y_d_0(t,x)=tau_y_0_1; % 管内第t个时间节点第x个空间节点处流体T_0、P_0下的屈服应力，Pa
            K_d_0(t,x)=K_0_1; % 管内第t个时间节点第x个空间节点处流体T_0、P_0下的稠度系数，Pa*s^n
            n_d_0(t,x)=n_0_1; % 管内第t个时间节点第x个空间节点处流体T_0、P_0下的流性指数
        end
    end
end
...
for t=Nt:1:(2*Nt-1)
    for x=Nx:-1:1
        if x>=(2*Nt-t) %% Nx-x<=t-Nt
            ...
            tau_y_a_0(t,x)=tau_y_0_2; % 环空第t个时间节点第x个空间节点处流体T_0、P_0下的屈服应力，Pa
            K_a_0(t,x)=K_0_2; % 环空第t个时间节点第x个空间节点处流体T_0、P_0下的稠度系数，Pa*s^n
            n_a_0(t,x)=n_0_2; % 环空第t个时间节点第x个空间节点处流体T_0、P_0下的流性指数
        else
            rho_a_0(t,x)=rho_0_1; % 环空第t个时间节点第x个空间节点处流体T_0、P_0下的密度，kg/m^3
            tau_y_a_0(t,x)=tau_y_0_1; % 环空第t个时间节点第x个空间节点处流体T_0、P_0下的屈服应力，Pa
            K_a_0(t,x)=K_0_1; % 环空第t个时间节点第x个空间节点处流体T_0、P_0下的稠度系数，Pa*s^n
            n_a_0(t,x)=n_0_1; % 环空第t个时间节点第x个空间节点处流体T_0、P_0下的流性指数
        end
        rho_d_0(t,x)=rho_0_2; % 管内第t个时间节点第x个空间节点处流体T_0、P_0下的密度，kg/m^3
        tau_y_d_0(t,x)=tau_y_0_2; % 管内第t个时间节点第x个空间节点处流体T_0、P_0下的屈服应力，Pa
        K_d_0(t,x)=K_0_2; % 管内第t个时间节点第x个空间节点处流体T_0、P_0下的稠度系数，Pa*s^n
...
OutPressure=1*10^6; % 环空出口压力，Pa

for t=1:1:Nt
    OP(t)=OutPressure;%*t; % 环空出口压力，Pa
end
for t=(Nt+1):1:(2*Nt-1)
    OP(t)=OutPressure; % 环空出口压力，Pa
end
...
%% 第1个时间节点相关参数计算（环空）
% 第1个空间节点处相关参数计算
P_a(1,1)=OutPressure; % 环空出口压力，Pa
rho_a(1,1)=Density_TP(rho_a_0(1,1),T_0,P_0,T_a(1,1),P_a(1,1)); % 环空出口流体密度，kg/m^3
Qv_a(1,1)=Qm_a_0(1,1)/rho_a(1,1); % 环空出口体积流量，m^3/s
V_a(1,1)=Qv_a(1,1)/A_a(1); % 环空出口流体流速，m/s
[tau_y_a(1,1),K_a(1,1),n_a(1,1)]=Rheology_TP(tau_y_a_0(1,1),K_a_0(1,1),n_a_0(1,1),T_0,P_0,T_a(1,1),P_a(1,1)); % 环空出口流体屈服应力（Pa）、稠度系数（Pa*s^n）、流性指数
[Ff_a(1,1),flow_pattern_a(1,1)]=Friction_annulus(rho_a(1,1),V_a(1,1),K_a(1,1),n_a(1,1),D_w(1),D_d_o(1)); % 环空出口单位长度摩擦压降（Pa/m）、流体流态（层流1、过渡流2、湍流3）
Fa_a(1,1)=0; % 环空出口单位长度加速度压降，Pa/m

% 第2～Nx个空间节点处相关参数计算
for x=2:1:Nx
    P_a_ass(1,x)=P_a(1,x-1)+rho_a(1,x-1)*g*cosd(theta(x-1))*dx(x-1); % 环空压力假设值，Pa
    ...
    while abs(ERROR_AnnPressure)>epsilon_e && COUNT_AnnPressure<epsilon_t
        ...
        [tau_y_a(1,x),K_a(1,x),n_a(1,x)]=Rheology_TP(tau_y_a_0(1,x),K_a_0(1,x),n_a_0(1,x),T_0,P_0,T_a(1,x),P_a_ass(1,x)); % 环空流体屈服应力（Pa）、稠度系数（Pa*s^n）、流性指数
        [Ff_a(1,x),flow_pattern_a(1,x)]=Friction_annulus(rho_a(1,x),V_a(1,x),K_a(1,x),n_a(1,x),D_w(x),D_d_o(x)); % 环空单位长度摩擦压降（Pa/m）、流体流态（层流1、过渡流2、湍流3）
        Fa_a(1,x)=abs(0.5*(rho_a(1,x-1)*V_a(1,x-1)^2-rho_a(1,x)*V_a(1,x)^2)); % 环空单位长度加速度压降，Pa/m
        P_a(1,x)=-rho_a(1,x)*V_a(1,x)^2+rho_a(1,x-1)*V_a(1,x-1)^2+P_a(1,x-1)-(((-rho_a(1,x-1)*g*cosd(theta(x-1))-Ff_a(1,x-1)-Fa_a(1,x-1))+(-rho_a(1,x)*g*cosd(theta(x))-Ff_a(1,x)-Fa_a(1,x)))/2)*dx(x-1); % 环空压力计算值，Pa
...
for t=2:1:(2*Nt-1)
    P_a(t,Nx)=P_a(t-1,Nx); % 环空井底压力假设值，Pa
    
    ERROR_AnnOutPressure=1; % 环空出口压力相对误差
    COUNT_AnnOutPressure=0; % 迭代次数
    while abs(ERROR_AnnOutPressure)>epsilon_e && COUNT_AnnOutPressure<epsilon_t
        COUNT_AnnOutPressure=COUNT_AnnOutPressure+1;
        
        % 第Nx个空间节点处相关参数计算
        rho_a(t,Nx)=Density_TP(rho_a_0(t,Nx),T_0,P_0,T_a(t,Nx),P_a(t,Nx)); % 环空井底流体密度，kg/m^3
        Qv_a(t,Nx)=Qm_a_0(t,Nx)/rho_a(t,Nx); % 环空井底体积流量，m^3/s
        V_a(t,Nx)=Qv_a(t,Nx)/A_a(Nx); % 环空井底流体流速，m/s
        [tau_y_a(t,Nx),K_a(t,Nx),n_a(t,Nx)]=Rheology_TP(tau_y_a_0(t,Nx),K_a_0(t,Nx),n_a_0(t,Nx),T_0,P_0,T_a(t,Nx),P_a(t,Nx)); % 环空井底流体屈服应力（Pa）、稠度系数（Pa*s^n）、流性指数
        [Ff_a(t,Nx),flow_pattern_a(t,Nx)]=Friction_annulus(rho_a(t,Nx),V_a(t,Nx),K_a(t,Nx),n_a(t,Nx),D_w(Nx),D_d_o(Nx)); % 环空井底单位长度摩擦压降（Pa/m）、流体流态（层流1、过渡流2、湍流3）
        Fa_a(t,Nx)=0; % 环空井底单位长度加速度压降，Pa/m
        
        % 第Nx-1～1个空间节点处相关参数计算
        for x=Nx-1:-1:1
            P_a_ass(t,x)=P_a(t,x+1)-rho_a(t,x+1)*g*cosd(theta(x+1))*dx(x); % 环空压力假设值，Pa
            ...
            while abs(ERROR_AnnPressure)>epsilon_e && COUNT_AnnPressure<epsilon_t
                ...
                [tau_y_a(t,x),K_a(t,x),n_a(t,x)]=Rheology_TP(tau_y_a_0(t,x),K_a_0(t,x),n_a_0(t,x),T_0,P_0,T_a(t,x),P_a_ass(t,x)); % 环空流体屈服应力（Pa）、稠度系数（Pa*s^n）、流性指数
                [Ff_a(t,x),flow_pattern_a(t,x)]=Friction_annulus(rho_a(t,x),V_a(t,x),K_a(t,x),n_a(t,x),D_w(x),D_d_o(x)); % 环空流体单位长度摩擦压降（Pa/m）、流体流态（层流1、过渡流2、湍流3）
                Fa_a(t,x)=abs(0.5*(rho_a(t,x)*V_a(t,x)^2-rho_a(t,x+1)*V_a(t,x+1)^2)); % 环空单位长度加速度压降，Pa/m
        ...
        ERROR_AnnOutPressure=abs(P_a(t,1)-OP(t))/OP(t); % 环空出口压力相对误差
        P_a(t,Nx)=P_a(t,Nx)-(P_a(t,1)-OP(t)); % 新的环空井底压力假设值，Pa
    end
end

%% 第1～（2*Nt-1）个时间节点钻头压降计算
for t=1:1:(2*Nt-1)
%     [delta_P_bit(t),V_nozzle(t)]=PressureDrop_Bit(C,d_nozzle,n_nozzle,Qv_a(t,Nx),rho_a(t,Nx)); % 钻头压降（Pa）、喷嘴流速（m/s）
    delta_P_bit(t)=0*10*10^6; % 钻头压降，Pa
end

%% 第1个时间节点相关参数计算（管内）
% 第Nx个空间节点处相关参数计算
P_d(1,Nx)=P_a(1,Nx)+delta_P_bit(1); % 管内井底压力，Pa
rho_d(1,Nx)=Density_TP(rho_d_0(1,Nx),T_0,P_0,T_d(1,Nx),P_d(1,Nx)); % 管内井底流体密度，kg/m^3
Qv_d(1,Nx)=Qm_d_0(1,Nx)/rho_d(1,Nx); % 管内井底体积流量，m^3/s
V_d(1,Nx)=Qv_d(1,Nx)/A_d(Nx); % 管内井底流体流速，m/s
[tau_y_d(1,Nx),K_d(1,Nx),n_d(1,Nx)]=Rheology_TP(tau_y_d_0(1,Nx),K_d_0(1,Nx),n_d_0(1,Nx),T_0,P_0,T_d(1,Nx),P_d(1,Nx)); % 管内井底流体屈服应力（Pa）、稠度系数（Pa*s^n）、流性指数
[Ff_d(1,Nx),flow_pattern_d(1,Nx)]=Friction_drillpipe(rho_d(1,Nx),V_d(1,Nx),K_d(1,Nx),n_d(1,Nx),D_d_i(Nx)); % 管内井底单位长度摩擦压降（Pa/m）、流体流态（层流1、过渡流2、湍流3）
Fa_d(1,Nx)=0; % 管内井底单位长度加速度压降，Pa/m

% 第Nx-1～1个空间节点处相关参数计算
for x=Nx-1:-1:1
    P_d_ass(1,x)=P_d(1,x+1)-rho_d(1,x+1)*g*cosd(theta(x+1))*dx(x); % 管内压力假设值，Pa
    ...
    while abs(ERROR_PipPressure)>epsilon_e && COUNT_PipPressure<epsilon_t
        ...
        [tau_y_d(1,x),K_d(1,x),n_d(1,x)]=Rheology_TP(tau_y_d_0(1,x),K_d_0(1,x),n_d_0(1,x),T_0,P_0,T_d(1,x),P_d_ass(1,x)); % 管内流体屈服应力（Pa）、稠度系数（Pa*s^n）、流性指数
        [Ff_d(1,x),flow_pattern_d(1,x)]=Friction_drillpipe(rho_d(1,x),V_d(1,x),K_d(1,x),n_d(1,x),D_d_i(x)); % 管内单位长度摩擦压降（Pa/m）、流体流态（层流1、过渡流2、湍流3）
        Fa_d(1,x)=abs(0.5*(rho_d(1,x+1)*V_d(1,x+1)^2-rho_d(1,x)*V_d(1,x)^2)); % 管内单位长度加速度压降，Pa/m
        P_d(1,x)=-rho_d(1,x)*V_d(1,x)^2+rho_d(1,x+1)*V_d(1,x+1)^2+P_d(1,x+1)-(((rho_d(1,x+1)*g*cosd(theta(x+1))-Ff_d(1,x+1)-Fa_d(1,x+1))+(rho_d(1,x)*g*cosd(theta(x))-Ff_d(1,x)-Fa_d(1,x)))/2)*dx(x); % 管内压力计算值，Pa
...
for t=2:1:(2*Nt-1)
    P_d_REAL(t,Nx)=P_a(t,Nx)+delta_P_bit(t); % 管内井底压力，Pa
    P_d(t,1)=P_d(t-1,1); % 入口压力假设值，Pa
    
    ERROR_PipOutPressure=1; % 管内井底压力相对误差
    COUNT_PipOutPressure=0; % 迭代次数
    while abs(ERROR_PipOutPressure)>epsilon_e && COUNT_PipOutPressure<epsilon_t
        COUNT_PipOutPressure=COUNT_PipOutPressure+1;
        
        % 第1个空间节点处相关参数计算
        rho_d(t,1)=Density_TP(rho_d_0(t,1),T_0,P_0,T_d(t,1),P_d(t,1)); % 入口流体密度，kg/m^3
        Qv_d(t,1)=Qm_d_0(t,1)/rho_d(t,1); % 入口体积流量，m^3/s
        V_d(t,1)=Qv_d(t,1)/A_d(1); % 入口流体流速，m/s
        [tau_y_d(t,1),K_d(t,1),n_d(t,1)]=Rheology_TP(tau_y_d_0(t,1),K_d_0(t,1),n_d_0(t,1),T_0,P_0,T_d(t,1),P_d(t,1)); % 入口流体屈服应力（Pa）、稠度系数（Pa*s^n）、流性指数
        [Ff_d(t,1),flow_pattern_d(t,1)]=Friction_drillpipe(rho_d(t,1),V_d(t,1),K_d(t,1),n_d(t,1),D_d_i(1)); % 入口单位长度摩擦压降（Pa/m）、流体流态（层流1、过渡流2、湍流3）
        Fa_d(t,1)=0; % 入口单位长度加速度压降，Pa/m
        
        % 第2～Nx个空间节点处相关参数计算
        for x=2:1:Nx
            P_d_ass(t,x)=P_d(t,x-1)+rho_d(t,x-1)*g*cosd(theta(x-1))*dx(x-1); % 管内压力假设值，Pa
            ...
            while abs(ERROR_PipPressure)>epsilon_e && COUNT_PipPressure<epsilon_t
                ...
                [tau_y_d(t,x),K_d(t,x),n_d(t,x)]=Rheology_TP(tau_y_d_0(t,x),K_d_0(t,x),n_d_0(t,x),T_0,P_0,T_d(t,x),P_d_ass(t,x)); % 管内流体屈服应力（Pa）、稠度系数（Pa*s^n）、流性指数
                [Ff_d(t,x),flow_pattern_d(t,x)]=Friction_drillpipe(rho_d(t,x),V_d(t,x),K_d(t,x),n_d(t,x),D_d_i(x)); % 管内单位长度摩擦压降（Pa/m）、流体流态（层流1、过渡流2、湍流3）
                Fa_d(t,x)=abs(0.5*(rho_d(t,x)*V_d(t,x)^2-rho_d(t,x-1)*V_d(t,x-1)^2)); % 管内单位长度加速度压降，Pa/m
        ...
        ERROR_PipOutPressure=abs(P_d(t,Nx)-P_d_REAL(t,Nx))/P_d_REAL(t,Nx); % 管内井底压力相对误差
        P_d(t,1)=P_d(t,1)-(P_d(t,Nx)-P_d_REAL(t,Nx)); % 新的入口压力假设值，Pa
    end
end
