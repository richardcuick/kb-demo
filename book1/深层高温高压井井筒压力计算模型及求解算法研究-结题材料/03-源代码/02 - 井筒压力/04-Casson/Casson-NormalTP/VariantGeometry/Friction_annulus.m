function [Ff_a,flow_pattern_a]=Friction_annulus(rho_a,V_a,eta_infinity_a,tau_y_c_a,D_w,D_do,ML)
    ...
    % 输入参数：rho_a为环空中钻井液密度（kg/m^3），V_a为环空中钻井液流速（m/s），A_a为环空内截面积（m^2），eta_infinity_a为环空中钻井液的卡森黏度（Pa·s），tau_y_c_a为环空中钻井液的卡森屈服应力（Pa），D_w为井筒内径（m），D_do为钻杆外径（m），ML为测试温度、压力下的钻井液质量排量（kg/s）
    % 输出参数：Ff_a为环空中单位长度上的摩擦压降（Pa/m），flow_pattern_a为环空中钻井液流型
    
    QL=ML/rho_a; % 钻井液体积排量=钻井液质量流量/钻井液密度
    D_h=D_w-D_do; % 水力直径（m）
