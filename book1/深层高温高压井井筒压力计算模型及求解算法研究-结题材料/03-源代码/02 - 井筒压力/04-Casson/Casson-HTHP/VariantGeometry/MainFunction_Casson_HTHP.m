...
[tau_y_c_0,eta_infinity_0]=Rheology_Casson(data_rheology); % 根据测试温度、压力条件下的卡森流体六速粘度计数据，采用回归法计算得到卡森屈服应力tau_y_c（Pa）和卡森黏度eta_infinity（Pa·s）
...
for i=1:Nx
    ...
    if i>46
        D_do(i)=0.0889; % D_do为钻杆外径（m）
        D_di(i)=0.076; % D_di为钻杆内径（m）
    end
...
%% 环空压力计算
% 1代表井口，Nx代表井底
% P_a为环空压力（Pa）,V_a为环空内钻井液速度（m/s），A_a为环空内截面积（m^2），tau_y_c_a为环空内钻井液的卡森屈服应力（Pa），eta_infinity_a为环空内钻井液的卡森黏度（Pa·s），Ff_a为环空内单位长度上的压降（Pa/m）
...
[tau_y_c_a(1),eta_infinity_a(1)]=Rheology_TP(tau_y_c_0,eta_infinity_0,T_0,P_0,T_a(1),P_a(1)); % 计算环空中当前网格的卡森屈服应力和卡森黏度
[Ff_a(1),flow_pattern_a(1)]=Friction_annulus(rho_a(1),V_a(1),eta_infinity_a(1),tau_y_c_a(1),D_w(1),D_do(1),Qm_0); % 计算环空中当前网格的单位长度压降和流型（层流、过渡流、湍流）
...
for i=2:Nx
    ...
    while abs(err_P)>1e-4 % 压力收敛的判断标准
        ...
        [tau_y_c_a(i),eta_infinity_a(i)]=Rheology_TP(tau_y_c_0,eta_infinity_0,T_0,P_0,T_a(i),P_a(i)); % 计算环空中当前网格的卡森屈服应力和卡森黏度
        [Ff_a(i),flow_pattern_a(i)]=Friction_annulus(rho_a(i),V_a(i),eta_infinity_a(i),tau_y_c_a(i),D_w(i),D_do(i),Qm_0); % 计算环空中当前网格的单位长度压降和流型（层流、过渡流、湍流）
...
Friction_annulus(1)=0; % Friction_annulus为环空摩擦压降
for i=2:Nx
    Friction_annulus(i)=Friction_annulus(i-1)+Ff_a(i)*dz(i-1); % 沿井深对环空摩擦压降累计求和
end
...
%% 钻杆压力计算
% 1代表井口，Nx代表井底
% P_d为钻杆压力（Pa）,V_d为钻杆内钻井液速度（m/s），A_d为钻杆内截面积（m^2），tau_y_c_d为钻杆内钻井液的卡森屈服应力（Pa），eta_infinity_d为钻杆内钻井液的卡森黏度（Pa·s），Ff_d为钻杆内单位长度上的压降（Pa/m）
...
[tau_y_c_d(Nx),eta_infinity_d(Nx)]=Rheology_TP(tau_y_c_0,eta_infinity_0,T_0,P_0,T_d(Nx),P_d(Nx)); % 计算钻杆中当前网格的卡森屈服应力、卡森粘度
[Ff_d(Nx),flow_pattern_d(Nx)]=Friction_drillpipe(rho_d(Nx),V_d(Nx),eta_infinity_d(Nx),tau_y_c_d(Nx),D_di(Nx),Qm_0); % 计算钻杆中当前网格的单位长度压降和流型（层流、过渡流、湍流）
...
for i=Nx-1:-1:1
    ...
    while abs(err_P)>1e-4 % 压力收敛的判断标准
        ...
        [tau_y_c_d(i),eta_infinity_d(i)]=Rheology_TP(tau_y_c_0,eta_infinity_0,T_0,P_0,T_d(i),P_d(i)); % 计算钻杆中当前网格的卡森屈服应力、卡森粘度
        [Ff_d(i),flow_pattern_d(i)]=Friction_drillpipe(rho_d(i),V_d(i),eta_infinity_d(i),tau_y_c_d(i),D_di(i),Qm_0); % 计算钻杆中当前网格的单位长度压降和流型（层流、过渡流、湍流）
...
Friction_drillpipe(1)=0; % Friction_drillpipe为钻杆摩擦压降
for i=2:Nx
    Friction_drillpipe(i)=Friction_drillpipe(i-1)+Ff_d(i)*dz(i-1); % 沿井深对钻杆摩擦压降累计求和
end
...
fprintf('钻柱摩擦压降（MPa）：%8.5f\n',Friction_drillpipe(Nx)/10^6); % 输出钻杆内摩擦压降，以MPa为单位
fprintf('环空摩擦压降（MPa）：%8.5f\n',Friction_annulus(Nx)/10^6); % 输出环空内摩擦压降，以MPa为单位
...
plot(Friction_drillpipe/10^6,Depth,'r-','LineWidth',2); % 绘制钻杆内钻井液压耗随井深变化曲线
hold on; % 保持曲线
plot(Friction_annulus/10^6,Depth,'b-.','LineWidth',2); % 绘制环空内钻井液压耗随井深变化曲线
hold off; % 替换曲线
set(gca,'YDir','reverse'); % 对Y坐标轴反转
...
set(gca,'xtick',[1 2 3]); % 设置X轴坐标轴刻度，1代表层流，2代表过渡流，3代表湍流。
