function [Ff_a,flow_pattern_a]=Friction_annulus(rho_a,V_a,mu_p_a,tau_y,D_w,D_d_o,ML)
    % 根据环空结构、钻井液性质和流动参数，计算环空摩擦压降及流型
    % 输入参数：rho_a为环空中钻井液密度（kg/m^3），V_a为环空中钻井液流速（m/s），mu_p_a为环空中钻井液的塑性黏度（Pa·s），tau_y为环空中钻井液的屈服应力（Pa），D_w为井筒内径（m），D_d_o为钻杆外径（m），ML为测试温度、压力下的钻井液质量排量（kg/s）
    ...
    while abs(err_tau)>1e-4 % 壁面处屈服应力收敛的判断标准
        tau_w_new=8*mu_p_a*QL/(pi*((D_w-D_d_o)/2)^2*(D_w+D_d_o)/2)+3/2*tau_y-1/2*tau_y^3/tau_w_old^2; % 计算新的壁面屈服应力值
