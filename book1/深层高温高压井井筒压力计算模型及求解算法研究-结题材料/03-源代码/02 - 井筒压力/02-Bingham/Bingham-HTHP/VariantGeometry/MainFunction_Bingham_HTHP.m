...
[tau_y,mu_p_0]=Rheology_Bingham(data_rheology); % 根据测试温度、压力条件下的宾汉流体六速粘度计数据，采用回归法计算得到剪切应力tau_y（Pa）和塑性黏度mu_p_0（Pa·s）
...
for i=1:Nx
    ...
    if i>46
        D_d_o(i)=0.0889; % D_do为钻杆外径（m）
        D_d_i(i)=0.076; % D_di为钻杆内径（m）
    end
...
Qm_0=Qv_0*rho_0; % ML_0为测试温度、压力下的钻井液质量排量（kg/s）
...
mu_p_a(1)=Rheology_TP(mu_p_0,T_0,P_0,T_a(1),P_a(1)); % 计算环空中当前网格的塑性黏度
[Ff_a(1),flow_pattern_a(1)]=Friction_annulus(rho_a(1),V_a(1),mu_p_a(1),tau_y,D_w(1),D_d_o(1),Qm_0); % 计算环空中当前网格的单位长度压降和流型（层流、湍流）
...
for i=2:Nx
    ...
    while abs(err_P)>1e-4 % 压力收敛的判断标准
        ...
        mu_p_a(i)=Rheology_TP(mu_p_0,T_0,P_0,T_a(i),P_a(i)); % 计算当前温度、压力下的环空钻井液塑性黏度
        [Ff_a(i),flow_pattern_a(i)]=Friction_annulus(rho_a(i),V_a(i),mu_p_a(i),tau_y,D_w(i),D_d_o(i),Qm_0); % 计算环空中当前网格的单位长度压降和流型（层流、湍流）
...
mu_p_d(Nx)=Rheology_TP(mu_p_0,T_0,P_0,T_d(Nx),P_d(Nx)); % 计算钻杆中当前网格的塑性黏度
[Ff_d(Nx),flow_pattern_d(Nx)]=Friction_drillpipe(rho_d(Nx),V_d(Nx),mu_p_d(Nx),tau_y,D_d_i(Nx),Qm_0); % 计算钻杆中当前网格的单位长度压降和流型（层流、湍流）
...
for i=Nx-1:-1:1
    ...
    while abs(err_P)>1e-4 % 压力收敛的判断标准
        ...
        mu_p_d(i)=Rheology_TP(mu_p_0,T_0,P_0,T_d(i),P_d(i)); % 计算当前温度、压力下的钻杆内钻井液塑性黏度
        [Ff_d(i),flow_pattern_d(i)]=Friction_drillpipe(rho_d(i),V_d(i),mu_p_d(i),tau_y,D_d_i(i),Qm_0); % 计算钻杆中当前网格的单位长度压降和流型（层流、湍流）
