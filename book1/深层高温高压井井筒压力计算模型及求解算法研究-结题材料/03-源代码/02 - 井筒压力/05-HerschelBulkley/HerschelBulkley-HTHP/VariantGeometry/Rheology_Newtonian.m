function mu=Rheology_Newtonian(data_rheology)
    ...
    %% Wellplan中，并非使用回归法计算粘度，当存在600转读数时，使用600转读数的1/2作为黏度；当不存在600转读数，且存在300转读数时，使用300转读数作为黏度
    if max(N)>=600 % 当存在600转读数时，使用600转读数的1/2作为粘度
        flag=1;
    elseif max(N)<300 % 不存在300转和600转读数时，默认为24mPa·s
        flag=2; 
    else % 当不存在600转读数且存在300转读数时，使用300转读数作为粘度
        flag=3; 
    end
    if flag==1 % 当存在600转读数时，使用300转读数作为粘度
        for i=1:m
            if N(i)==600
                mu=theta(i)/2/1000; % mPa·s转换为Pa·s
            end
        end
    elseif flag==3 % 当不存在600转读数且存在300转读数时，使用300转读数作为粘度
        for i=1:m
            if N(i)==300
                mu=theta(i)/1000; % mPa·s转换为Pa·s
            end
        end
    elseif flag==2 % 不存在300转和600转读数时，默认为24mPa·s
        mu=24/1000; % mPa·s转换为Pa·s
    end
end