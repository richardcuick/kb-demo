function [Ff_d,flow_pattern_d]=Friction_drillpipe(rho_d,V_d,mu_d,D_d_i)
    % 根据钻杆结构、钻井液性质和流动参数，计算钻杆摩擦压降及流型
    % 输入参数：rho_d为钻杆中钻井液密度（kg/m^3），V_d为钻杆中钻井液流速（m/s），mu_d为钻杆中钻井液的塑性黏度（Pa·s），D_di为钻杆内径（m）
    ...
    Ff_d=2*f_d*V_d^2*rho_d/D_d_i; % 计算环空内钻井液在单位长度上的摩擦压降
end