function [PressureSurge_a,flow_pattern_a]=PressureSurgeAnnulus(rho_a,V_a,eta_infinity_a,tau_y_c_a,D_w,D_d_o)
    ...
    if Re_g_a==0 % 流体静止
        f_a=0; % 计算流体静止条件下，环空内钻井液摩擦因子
        PressureSurge_a=0; % 计算流体静止条件下，环空内钻井液在单位长度上的波动压力，Pa/m
        flow_pattern_a=0; % flow_pattern_a为环空流型，0表示流体静止
    elseif Re_g_a>0 && Re_g_a<=Re_gcl_a % 层流
