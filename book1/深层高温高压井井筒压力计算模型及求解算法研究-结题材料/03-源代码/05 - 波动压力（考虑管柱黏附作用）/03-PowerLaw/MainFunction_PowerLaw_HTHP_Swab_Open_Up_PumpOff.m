...
[K_0,n_0]=Rheology_PowerLaw(data_rheology); % 计算钻井液稠度系数K_0（Pa·s^n）和流性指数n_0
K_0=2; % 稠度系数，Pa·s^n
n_0=0.6; % 流性指数
...
while abs(err_BPD)>1*10^(-4) && COUNT_Q<1*10^3
    ...
    for i=2:1:Nx_bot
        ...
        while err_P>1*10^(-4) && COUNT_P<1*10^3
            ...
            [K_d(i),n_d(i)]=Rheology_TP(K_0,n_0,T_0,P_0,T_d(i),P_d_ass(i)); % 井口钻井液稠度系数（Pa·s^n）和流性指数
            [PressureSurge_d(i),flow_pattern_d(i)]=PressureSurgeDrillPipe(rho_d(i),V_d(i),K_d(i),n_d(i),D_d_i(i)); % 井口流体单位长度波动压力（Pa/m）和流体流型（层流、过渡流、湍流）
