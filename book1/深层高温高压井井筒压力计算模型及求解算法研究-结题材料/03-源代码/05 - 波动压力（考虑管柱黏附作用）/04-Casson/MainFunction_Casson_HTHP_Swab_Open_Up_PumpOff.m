...
Qv=V_p*(0.25*pi*(D_d_o(Nx)^2-D_d_i(Nx)^2))+0.5*V_p*(A_a(Nx)+A_d(Nx)); % 管柱起出引起的流量，m^3/s
...
% a(1)=Qv_a_ass;
% b(1)=err_BPD;
% while abs(err_BPD)>1*10^(-1) && COUNT_Q<1*10^3
%     COUNT_Q=COUNT_Q+1;  % 出口压力迭代次数
    ...
    [tau_y_c_d(1),eta_infinity_d(1)]=Rheology_TP(tau_y_c_0,eta_infinity_0,T_0,P_0,T_d(1),P_d(1)); % 井口钻井液卡森屈服应力（Pa）和卡森黏度（Pa*s）
    [PressureSurge_d(1),flow_pattern_d(1)]=PressureSurgeDrillPipe(rho_d(1),V_d(1),eta_infinity_d(1),tau_y_c_d(1),D_d_i(1)); % 井口流体单位长度波动压力（Pa/m）和流体流型（层流、过渡流、湍流）
    ...
    for i=2:1:Nx_bot
        ...
        while err_P>1*10^(-4) && COUNT_P<1*10^3
            ...
            [tau_y_c_d(i),eta_infinity_d(i)]=Rheology_TP(tau_y_c_0,eta_infinity_0,T_0,P_0,T_d(i),P_d_ass(i)); % 管内钻井液卡森屈服应力（Pa）和卡森黏度（Pa*s）
            [PressureSurge_d(i),flow_pattern_d(i)]=PressureSurgeDrillPipe(rho_d(i),V_d(i),eta_infinity_d(i),tau_y_c_d(i),D_d_i(i)); % 管内流体单位长度波动压力（Pa/m）和流体流型（层流、过渡流、湍流）
