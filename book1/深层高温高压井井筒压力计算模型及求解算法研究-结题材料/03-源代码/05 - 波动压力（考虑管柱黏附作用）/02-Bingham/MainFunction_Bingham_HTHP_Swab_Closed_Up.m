...
V_a(1)=V_p*((D_bot^2)/(D_w(1)^2-D_d_o(1)^2)+0.5); % 井口钻井液流速，m/s
...
PressureSurge_a(1)=-PressureSurge_a(1); % 钻井液单位长度波动压力（Pa/m）
...
for i=2:1:Nx_bot
    ...
    while err_P>1*10^(-4) && COUNT_P<1*10^3
        ...
        V_a(i)=V_p*((D_bot^2)/(D_w(i)^2-D_d_o(i)^2)+0.5); % 环空钻井液流速，m/s
        ...
        PressureSurge_a(i)=-PressureSurge_a(i); % 钻井液单位长度波动压力（Pa/m）
