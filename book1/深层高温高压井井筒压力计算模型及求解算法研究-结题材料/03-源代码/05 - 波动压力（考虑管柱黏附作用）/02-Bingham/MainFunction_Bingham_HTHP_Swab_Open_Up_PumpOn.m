...
V_a(1)=V_p*((D_bot^2)/(D_w(1)^2-D_d_o(1)^2)+0.5)+Qm/(rho_a(1)*A_a(1)); % 井口钻井液流速，m/s
...
for i=2:1:Nx_bot
    ...
    while err_P>1*10^(-4) && COUNT_P<1*10^3
        ...
        V_a(i)=V_p*((D_bot^2)/(D_w(i)^2-D_d_o(i)^2)+0.5)+Qm/(rho_a(i)*A_a(i)); % 环空钻井液流速，m/s
