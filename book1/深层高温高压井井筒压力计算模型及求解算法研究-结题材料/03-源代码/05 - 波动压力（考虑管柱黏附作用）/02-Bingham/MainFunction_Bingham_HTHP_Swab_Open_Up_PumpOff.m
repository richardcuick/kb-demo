...
%% 基础数据及预处理（国际单位制）
data_rheology=csvread('RheologyData.csv',1,0); % data_rheology存储测试温度和压力条件下的宾汉流体六速粘度计数据，第一列为转速，第二列为读数；因存在表头且csv只能读取纯数据，1代表从第2行开始读取，0代表从第1列开始读取
[tau_y,mu_p_0]=Rheology_Bingham(data_rheology); % 计算钻井液屈服应力tau_y（Pa）和塑性黏度mu_p_0（Pa·s）
tau_y=5; % 屈服应力，Pa
mu_p_0=50*10^(-3); % 塑性黏度，Pa·s
...
Qv=V_p*(0.25*pi*(D_d_o(Nx)^2-D_d_i(Nx)^2))+0.5*V_p*(A_a(Nx)+A_d(Nx))+0.5*V_p*(A_a(Nx)+A_d(Nx)); % 管柱起出引起的流量，m^3/s
...
Qv_a_ass=((D_w(Nx)^2-D_d_o(Nx)^2)/(D_w(Nx)^2-D_d_o(Nx)^2+D_d_i(Nx)^2))*Qv; % 环空流量假设值，m^3/s
...
%     COUNT_Q=COUNT_Q+1;  % 出口压力迭代次数
    ...
    mu_p_a(1)=Rheology_TP(mu_p_0,T_0,P_0,T_a(1),P_a(1)); % 井口钻井液塑性粘度，Pa*s
    [PressureSurge_a(1),flow_pattern_a(1)]=PressureSurgeAnnulus(rho_a(1),V_a(1),mu_p_a(1),tau_y,D_w(1),D_d_o(1)); % 井口钻井液单位长度波动压力（Pa/m）和流体流型（层流、湍流）
    PressureSurge_a(1)=-PressureSurge_a(1); % 井口钻井液单位长度波动压力，Pa/m
    ...
    for i=2:1:Nx_bot
        ...
        while err_P>1*10^(-4) && COUNT_P<1*10^3
            ...
            mu_p_a(i)=Rheology_TP(mu_p_0,T_0,P_0,T_a(i),P_a_ass(i)); % 环空钻井液塑性粘度，Pa*s
            [PressureSurge_a(i),flow_pattern_a(i)]=PressureSurgeAnnulus(rho_a(i),V_a(i),mu_p_a(i),tau_y,D_w(i),D_d_o(i)); % 环空钻井液单位长度波动压力（Pa/m）和流体流型（层流、湍流）
            PressureSurge_a(i)=-PressureSurge_a(i); % 环空钻井液单位长度波动压力，Pa/m
    ...
    mu_p_d(1)=Rheology_TP(mu_p_0,T_0,P_0,T_d(1),P_d(1)); % 井口钻井液塑性粘度，Pa*s
    [PressureSurge_d(1),flow_pattern_d(1)]=PressureSurgeDrillPipe(rho_d(1),V_d(1),mu_p_d(1),tau_y,D_d_i(1)); % 井口钻井液单位长度波动压力（Pa/m）和流体流型（层流、湍流）
    PressureSurge_d(1)=-PressureSurge_d(1); % 井口钻井液单位长度波动压力，Pa/m
    ...
    for i=2:1:Nx_bot
        ...
        while err_P>1*10^(-4) && COUNT_P<1*10^3
            ...
            mu_p_d(i)=Rheology_TP(mu_p_0,T_0,P_0,T_d(i),P_d_ass(i)); % 管内钻井液塑性粘度，Pa*s
            [PressureSurge_d(i),flow_pattern_d(i)]=PressureSurgeDrillPipe(rho_d(i),V_d(i),mu_p_d(i),tau_y,D_d_i(i)); % 管内钻井液单位长度波动压力（Pa/m）和流体流型（层流、湍流）
            PressureSurge_d(i)=-PressureSurge_d(i); % 环空钻井液单位长度波动压力，Pa/m
