function [result] = Function_ParameterSol12(data)
    ...
    for i=1:1:n_row
       parameter_model(i,1)=parameter_0*exp(INTERCEPT+ALPHA*(data(i,1)-T_0)+BETA*(data(i,1)-T_0)^2+GAMMA*(data(i,2)-P_0)+DELTA*((data(i,1)-T_0)*(data(i,2)-P_0))); % 求流体参数模型预测值
    end
...
%         legend('Actual Value','Predicted Value');
    ...
    elseif ZRESID_OverTwoTimes>0 % 如果有离群值
        ...
        for i=1:1:n_row_new
            parameter_model_new(i,1)=parameter_0_new*exp(INTERCEPT_new+ALPHA_new*(data_new(i,1)-T_0_new)+BETA_new*(data_new(i,1)-T_0_new)^2+GAMMA_new*(data_new(i,2)-P_0_new)+DELTA_new*((data_new(i,1)-T_0_new)*(data_new(i,2)-P_0_new))); % 求流体参数模型预测值
        end
...
%         fprintf('MRE(Percent): %8.6f\n',mre_new); % 平均相对误差
        ...
        result(3,1)=parameter_0_new; % T_0、P_0下流体参数
