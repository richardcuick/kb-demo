function [result] = Function_ParameterSol08(data)
    ...
    for i=1:1:n_row
       parameter_model(i,1)=parameter_0*exp(ALPHA*(data(i,1)-T_0)+BETA*(data(i,2)-P_0)+GAMMA*(data(i,1)-T_0)^2); % 求流体参数模型预测值
    end
...
%         legend('Actual Value','Predicted Value');
    ...
    elseif ZRESID_OverTwoTimes>0 % 如果有离群值
        ...
        for i=1:1:n_row_new
            ...
            DATA_new(i,3)=(data_new(i,1)-T_0_new)^2;
        ...
        for i=1:1:n_row_new
            parameter_model_new(i,1)=parameter_0_new*exp(ALPHA_new*(data_new(i,1)-T_0_new)+BETA_new*(data_new(i,2)-P_0_new)+GAMMA_new*(data_new(i,1)-T_0_new)^2); % 求流体参数模型预测值
        end
