function [result] = Function_ParameterSol09(data)
    % 输入：数据data存储不同温度、压力及相应流体参数（共3列），其中第一列为温度（℃），第二列为压力（MPa），最后一列为相应流体参数
    % 输出：result依次存储参考温度（℃）、参考压力（MPa）、常数C、系数ALPHA、系数BETA、决定系数、平均相对误差
    ...
    for i=1:1:n_row
       parameter_model(i,1)=exp(ALPHA*(data(i,1)-T_0)+BETA*(data(i,2)-P_0)+INTERCEPT); % 求流体参数模型预测值
    end
...
%         legend('Actual Value','Predicted Value');
    ...
    elseif ZRESID_OverTwoTimes>0 % 如果有离群值
        ...
        for i=1:1:n_row_new
            parameter_model_new(i,1)=exp(ALPHA_new*(data_new(i,1)-T_0_new)+BETA_new*(data_new(i,2)-P_0_new)+INTERCEPT_new); % 求流体参数模型预测值
        end
