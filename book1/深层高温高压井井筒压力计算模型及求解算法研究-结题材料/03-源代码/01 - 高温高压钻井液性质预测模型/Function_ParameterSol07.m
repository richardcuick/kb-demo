function [result] = Function_ParameterSol07(data)
    % 输入：数据data存储不同温度、压力及相应流体参数（共3列），其中第一列为温度（℃），第二列为压力（MPa），最后一列为相应流体参数
    % 输出：result依次存储参考温度（℃）、参考压力（MPa）、参考温压下流体参数、常数C、系数ALPHA、系数BETA、决定系数、平均相对误差
    ...
    for i=1:1:n_row
        ...
        DATA(i,3)=log(data(i,3)/parameter_0);
    end
    ...
    for i=1:1:n_row
       parameter_model(i,1)=INTERCEPT*parameter_0*exp(ALPHA*(data(i,1)-T_0)+BETA*(data(i,2)-P_0)); % 求流体参数模型预测值
    end
...
%         legend('Actual Value','Predicted Value');
    ...
    elseif ZRESID_OverTwoTimes>0 % 如果有离群值
        ...
        for i=1:1:n_row_new
            ...
            DATA_new(i,3)=log(data_new(i,3)/parameter_0_new);
        end
        ...
        for i=1:1:n_row_new
            parameter_model_new(i,1)=INTERCEPT_new*parameter_0_new*exp(ALPHA_new*(data_new(i,1)-T_0_new)+BETA_new*(data_new(i,2)-P_0_new)); % 求流体参数模型预测值
        end
