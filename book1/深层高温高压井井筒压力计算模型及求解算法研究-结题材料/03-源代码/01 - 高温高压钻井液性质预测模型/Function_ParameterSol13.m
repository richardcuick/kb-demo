function [result] = Function_ParameterSol13(data)
    % 输入：数据data存储不同温度、压力及相应流体参数（共3列），其中第一列为温度（℃），第二列为压力（MPa），最后一列为相应流体参数
    % 输出：result依次存储参考温度（℃）、参考压力（MPa）、参考温压下流体参数、常数C、系数ALPHA、系数BETA、系数GAMMA、系数DELTA、决定系数、平均相对误差
    ...
    for i=1:1:n_row
        ...
        DATA(i,2)=data(i,2)-P_0;
        DATA(i,3)=((data(i,1)-T_0)*(data(i,2)-P_0))/(data(i,1)*T_0);
        DATA(i,4)=((data(i,1)-T_0)^2)/(data(i,1)*T_0);
        DATA(i,5)=log(data(i,3)/parameter_0);
    end
    ...
    for i=1:1:n_row
       parameter_model(i,1)=INTERCEPT*parameter_0*exp(ALPHA*(data(i,1)-T_0)+BETA*(data(i,2)-P_0)+GAMMA*((data(i,1)-T_0)*(data(i,2)-P_0))/(data(i,1)*T_0)+DELTA*((data(i,1)-T_0)^2)/(data(i,1)*T_0)); % 求流体参数模型预测值
    end
...
%         legend('Actual Value','Predicted Value');
    ...
    elseif ZRESID_OverTwoTimes>0 % 如果有离群值
        ...
        for i=1:1:n_row_new
            ...
            DATA_new(i,2)=data_new(i,2)-P_0_new;
            DATA_new(i,3)=((data_new(i,1)-T_0_new)*(data_new(i,2)-P_0_new))/(data_new(i,1)*T_0_new);
            DATA_new(i,4)=((data_new(i,1)-T_0_new)^2)/(data_new(i,1)*T_0_new);
            DATA_new(i,5)=log(data_new(i,3)/parameter_0_new);
        end
        ...
        INTERCEPT_new=exp(beta_new(1,1));
        ...
        for i=1:1:n_row_new
            parameter_model_new(i,1)=INTERCEPT_new*parameter_0_new*exp(ALPHA_new*(data_new(i,1)-T_0_new)+BETA_new*(data_new(i,2)-P_0_new)+GAMMA_new*((data_new(i,1)-T_0_new)*(data_new(i,2)-P_0_new))/(data_new(i,1)*T_0_new)+DELTA_new*((data_new(i,1)-T_0_new)^2)/(data_new(i,1)*T_0_new)); % 求流体参数模型预测值
        end
...
%         fprintf('MRE(Percent): %8.6f\n',mre_new); % 平均相对误差
        ...
        result(4,1)=INTERCEPT_new; % 常数
        result(5,1)=ALPHA_new; % 系数
        result(6,1)=BETA_new; % 系数
        result(7,1)=GAMMA_new; % 系数
        result(8,1)=DELTA_new; % 系数
