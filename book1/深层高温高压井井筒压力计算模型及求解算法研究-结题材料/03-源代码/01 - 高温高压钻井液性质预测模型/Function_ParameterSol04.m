function [result] = Function_ParameterSol04(data)
    % 输入：数据data存储不同温度、压力及相应流体参数（共3列），其中第一列为温度（℃），第二列为压力（MPa），最后一列为相应流体参数
    % 输出：result依次存储参考温压下流体参数、常数C、系数ALPHA、系数BETA、决定系数、平均相对误差
    ...
    for i=1:1:n_row
       parameter_model(i,1)=INTERCEPT*parameter_0*exp((ALPHA*(data(i,1))^(-1))+BETA*(data(i,2))); % 求流体参数模型预测值
    end
...
%         fprintf('MRE(Percent): %8.6f\n',mre); % 平均相对误差
        ...
        result(1,1)=parameter_0; % T_0、P_0下流体参数
        result(2,1)=INTERCEPT; % 常数
        result(3,1)=ALPHA; % 系数
        result(4,1)=BETA; % 系数
        result(5,1)=r^2; % 决定系数
        result(6,1)=mre; % 平均相对误差
...
%         legend('Actual Value','Predicted Value');
    ...
    elseif ZRESID_OverTwoTimes>0 % 如果有离群值
        ...
        for i=1:1:n_row_new
            DATA_new(i,1)=(data_new(i,1))^(-1);
            DATA_new(i,2)=data_new(i,2);
        ...
        for i=1:1:n_row_new
            parameter_model_new(i,1)=INTERCEPT_new*parameter_0_new*exp((ALPHA_new*(data_new(i,1))^(-1))+BETA_new*(data_new(i,2))); % 求流体参数模型预测值
        end
...
%         fprintf('MRE(Percent): %8.6f\n',mre_new); % 平均相对误差
        ...
        result(1,1)=parameter_0_new; % T_0_new、P_0_new下流体参数
        result(2,1)=INTERCEPT_new; % 常数
        result(3,1)=ALPHA_new; % 系数
        result(4,1)=BETA_new; % 系数
        result(5,1)=r_new^2; % 决定系数
        result(6,1)=mre_new; % 平均相对误差
