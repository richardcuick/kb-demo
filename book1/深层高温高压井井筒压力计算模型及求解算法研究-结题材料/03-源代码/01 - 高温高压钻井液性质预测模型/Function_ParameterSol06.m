function [result] = Function_ParameterSol06(data)
    % 输入：数据data存储不同温度、压力及相应流体参数（共3列），其中第一列为温度（℃），第二列为压力（MPa），最后一列为相应流体参数
    % 输出：result依次存储参考温度（℃）、参考压力（MPa）、参考温压下流体参数、系数ALPHA、系数BETA、系数GAMMA、系数DELTA、系数EPSILON、决定系数、平均相对误差
    
    n=size(data); % 数据（自变量+因变量）共多少行多少列
    n_row=n(1,1); % 样本观测次数
    n_col=n(1,2); % 自变量+因变量个数，因变量只有一个，所以自变量有（n_col-1）个

    %% 以温度最小值且压力最小值时对应的温度、压力及流体参数作为模型基准参考值
    min_T=data(1,1);
    for i=1:1:n_row
        if min_T>=data(i,1)
    ...
    parameter_0=data(min_TP_location,3); % parameter_0为T_0、P_0下流体参数

    %% 数据预处理（将原始数据处理成多元线性方程中的x1、x2、···、xp、y）（data→DATA，data一定是个3列的数据，而DATA则是个≥3列的数据）
    for i=1:1:n_row
        DATA(i,1)=data(i,1)-T_0;
        DATA(i,2)=(data(i,1)-T_0)^2;
        DATA(i,3)=data(i,2)-P_0;
        DATA(i,4)=(data(i,2)-P_0)^2;
        DATA(i,5)=(data(i,1)-T_0)*(data(i,2)-P_0);
        DATA(i,6)=log(data(i,3)/parameter_0);
    end

    %% 计算多元线性回归系数beta及相关系数r
    [beta,r] = Function_MultipleLinearRegression_WithoutIntercept(DATA);
    ...
    for i=1:1:n_row
       parameter_model(i,1)=parameter_0*exp(ALPHA*(data(i,1)-T_0)+BETA*(data(i,1)-T_0)^2+GAMMA*(data(i,2)-P_0)+DELTA*(data(i,2)-P_0)^2+EPSILON*(data(i,1)-T_0)*(data(i,2)-P_0)); % 求流体参数模型预测值
    end
...
%         fprintf('parameter_0: %8.6f\n',parameter_0); % T_0、P_0下流体参数
%         fprintf('ALPHA: %8.6f\n',ALPHA); % 系数
%         fprintf('BETA: %8.6f\n',BETA); % 系数
%         fprintf('GAMMA: %8.6f\n',GAMMA); % 系数
%         fprintf('DELTA: %8.6f\n',DELTA); % 系数
...
%         fprintf('MRE(Percent): %8.6f\n',mre); % 平均相对误差
        ...
        result(3,1)=parameter_0; % T_0、P_0下流体参数
        result(4,1)=ALPHA; % 系数
        result(5,1)=BETA; % 系数
        result(6,1)=GAMMA; % 系数
        result(7,1)=DELTA; % 系数
...
%         legend('Actual Value','Predicted Value');
    ...
    elseif ZRESID_OverTwoTimes>0 % 如果有离群值
        ...
        parameter_0_new=data_new(min_TP_location_new,3); % parameter_0_new为T_0_new、P_0_new下流体参数

        %% 数据预处理（将原始数据处理成多元线性方程中的x1、x2、···、xp、y）（data_new→DATA_new，data_new一定是个3列的数据，而DATA_new则是个≥3列的数据）
        for i=1:1:n_row_new
            DATA_new(i,1)=data_new(i,1)-T_0_new;
            DATA_new(i,2)=(data_new(i,1)-T_0_new)^2;
            DATA_new(i,3)=data_new(i,2)-P_0_new;
            DATA_new(i,4)=(data_new(i,2)-P_0_new)^2;
            DATA_new(i,5)=(data_new(i,1)-T_0_new)*(data_new(i,2)-P_0_new);
            DATA_new(i,6)=log(data_new(i,3)/parameter_0_new);
        end

        %% 计算多元线性回归系数beta_new及相关系数r_new
        [beta_new,r_new] = Function_MultipleLinearRegression_WithoutIntercept(DATA_new);

        %% 将回归系数beta_new转化为模型中系数
        ALPHA_new=beta_new(1,1);
        BETA_new=beta_new(2,1);
        GAMMA_new=beta_new(3,1);
        DELTA_new=beta_new(4,1);
        EPSILON_new=beta_new(5,1);

        %% 计算相对误差re_new及平均相对误差mre_new
        parameter_model_new=zeros(n_row_new,1); % 流体参数模型预测值初值

        for i=1:1:n_row_new
            parameter_model_new(i,1)=parameter_0_new*exp(ALPHA_new*(data_new(i,1)-T_0_new)+BETA_new*(data_new(i,1)-T_0_new)^2+GAMMA_new*(data_new(i,2)-P_0_new)+DELTA_new*(data_new(i,2)-P_0_new)^2+EPSILON_new*(data_new(i,1)-T_0_new)*(data_new(i,2)-P_0_new)); % 求流体参数模型预测值
        end
...
%         fprintf('T_0(degree Celsius): %8.6f\n',T_0_new); % 参考温度，℃
%         fprintf('P_0(MPa): %8.6f\n',P_0_new); % 参考压力，MPa
%         fprintf('parameter_0: %8.6f\n',parameter_0_new); % T_0_new、P_0_new下流体参数
%         fprintf('ALPHA: %8.6f\n',ALPHA_new); % 常数
%         fprintf('BETA: %8.6f\n',BETA_new); % 系数
%         fprintf('GAMMA: %8.6f\n',GAMMA_new); % 系数
%         fprintf('DELTA: %8.6f\n',DELTA_new); % 系数
...
%         fprintf('MRE(Percent): %8.6f\n',mre_new); % 平均相对误差
        ...
        result(3,1)=parameter_0_new; % T_0_new、P_0_new下流体参数
